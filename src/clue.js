...
function pickMystery() {
  const rdmRoom = selectRandom(roomsArray),
    rdmSuspect = selectRandom(suspectsArray),
    rdmWeapon = selectRandom(weaponsArray),
    // 🧐 obj = new Object({ suspect: rdmSuspect, weapon: rdmWeapon, room: rdmRoom }), ♻️
    mystery = { suspect: rdmSuspect, weapon: rdmWeapon, room: rdmRoom };
  return mystery;
}
// ITERATION 3
const mystery = pickMystery();

const revealMystery = (quest) =>
  `${quest.suspect.firstName} ${quest.suspect.lastName} killed Mr. Boddy using the ${quest.weapon.name} in the ${quest.room.name}!`;
